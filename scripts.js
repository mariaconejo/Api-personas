...
const apiUrl = "https://603e6d9e48171b0017b2f707.mockapi.io/people";
...
formulario.addEventListener('submit', (e) => {
    e.preventDefault();
    const info = {
        'fullName': `${formulario.elements[0].value}`,
        'email': `${formulario.elements[1].value}`,
        'phoneNumber':`${formulario.elements[2].value}`,
        'city': `${formulario.elements[3].value}`,
    };
    e.target.reset();
    crearPersona(info);
});
...
const obtenerPersonaDatos = (person) => {
    return `
...
function getPeopleHtmlList(people) {
    return people
    .map((person) => {
        return obtenerPersonaDatos(person);
    })
    .join("");
}
...
function crearPersona(info){
    ...
    .then((data)=>{
        htmlListElement.innerHTML += obtenerPersonaDatos(data);
    })
